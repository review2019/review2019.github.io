...
if(screen.width >= 750){
    ...
    var empushyFeatures = ['appName', 'category', 'subject', 'opened']
    ...
    */
    ...
    function updateEpoch(nodes){
        empushyCurrentEpoch = empushyCurrentEpoch + ((25000))
    ...
    function bubbleChart(nodeColors) {
        ...
        function startEmpushyNotificationSimulation() {
            ...
            empushyNotificationSimulation = setInterval(function(){
                ...
                for(node of nodes){
                    ...
                    else if(empushyCurrentEpoch > node.posted && node.location == 'Unsent'){
                        agentAct(node)
                        emPushyRemoved++;
                    }
                ...
                if(emPushyRemoved >= nodes.length){
      ...
      chart.toggleDisplay = function (displayName) {
        ...
        else if(displayName == 'empushyReplay'){
    ...
    function empushy_demo(file_path){
        d3.json(file_path)
            .then(function(data) {
    ...
    function remove_empushy_demo(){
        $('#empushyVis').empty()
        $('#empushyDropdownLegend').empty()
        $('#empushyChooseFeature').empty()
        
    }

    function setUpChart(data){
        data.sor
        empushyStartingEpoch = data[0].posted - (1000*60*60*1) // start 5 hours early!
        empushyCurrentEpoch = empushyStartingEpoch
        updateEpoch();
        /*
